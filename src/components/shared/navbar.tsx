...
import { UserCircle, LogOut, Mail, ChevronDown, User } from "lucide-react";
      ...
      <div className="container-custom">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative flex items-center min-w-20 justify-end">
              {isPending && (
                <div className="flex items-center space-x-2 mr-8 animate-pulse">
                  {/* Skeleton avatar */}
                  <LoadingState />
                </div>
              )}
              ...
              {!isPending && !session && (
                <Link href="/auth/login">
                  <button className="px-4 py-2 bg-(--color-charcoal) text-white rounded-lg text-sm font-medium hover:bg-(--color-green) transition-all duration-300 hover:scale-105">
                    Login
                  </button>
                </Link>
              )}
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-gray-200 py-2 animate-fade-in">
                      ...
                      <div className="px-4 py-3 border-b border-gray-100">
                        <p className="text-sm font-medium text-(--color-charcoal) mb-1">
                          {session.user.name}
                        </p>
                      ...
                      <button
                        ...
                        className="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-gray-50 transition-colors flex items-center space-x-2 mt-1"
                      >
